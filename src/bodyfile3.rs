...
impl TryFrom<&str> for Bodyfile3Line {
    ...
    fn try_from(line: &str) -> Result<Self, Self::Error> {
        ...
        let inode = parts[2];
        let mode = parts[3];
        let uid = str::parse::<i64>(parts[4]).or(Err(Self::Error::IllegalUid))?;
        let gid = str::parse::<i64>(parts[5]).or(Err(Self::Error::IllegalGid))?;
        let size = str::parse::<i64>(parts[6]).or(Err(Self::Error::IllegalSize))?;
