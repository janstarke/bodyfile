...
impl TryFrom<&str> for Bodyfile3Line {
    ...
    fn try_from(line: &str) -> Result<Self, Self::Error> {
        ...
        match RE.captures(line) {
            None => return Err(Self::Error::WrongNumberOfColumns),
            Some(captures) => {
                if captures.len() != 12 {
                    return Err(Self::Error::WrongNumberOfColumns);
                }

                let md5 = captures.get(1).ok_or(Self::Error::IllegalHash)?.as_str();
                let name = captures.get(2).ok_or(Self::Error::IllegalName)?.as_str();
                let inode = captures.get(3).ok_or(Self::Error::IllegalInode)?.as_str();
                let mode = captures.get(4).ok_or(Self::Error::IllegalMode)?.as_str();
                let uid = str::parse::<u64>(captures.get(5).ok_or(Self::Error::IllegalUid)?.as_str()).or(Err(Self::Error::IllegalUid))?;
                let gid = str::parse::<u64>(captures.get(6).ok_or(Self::Error::IllegalGid)?.as_str()).or(Err(Self::Error::IllegalGid))?;
                
                let size = str::parse::<u64>(captures.get(7).ok_or(Self::Error::IllegalSize)?.as_str()).or(Err(Self::Error::IllegalSize))?;

                let atime = str::parse::<i64>(captures.get(8).ok_or(Self::Error::IllegalATime)?.as_str()).or(Err(Self::Error::IllegalATime))?;
                if atime < -1 { return Err(Self::Error::IllegalATime); }

                let mtime = str::parse::<i64>(captures.get(9).ok_or(Self::Error::IllegalMTime)?.as_str()).or(Err(Self::Error::IllegalMTime))?;
                if mtime < -1 { return Err(Self::Error::IllegalMTime); }

                let ctime = str::parse::<i64>(captures.get(10).ok_or(Self::Error::IllegalCTime)?.as_str()).or(Err(Self::Error::IllegalCTime))?;
                if ctime < -1 { return Err(Self::Error::IllegalCTime); }

                let crtime = str::parse::<i64>(captures.get(11).ok_or(Self::Error::IllegalCRTime)?.as_str()).or(Err(Self::Error::IllegalCRTime))?;
